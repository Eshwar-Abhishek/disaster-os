...
import { Shield, ShieldAlert, HeartPulse, Lock, Mail, ArrowRight, KeyRound, Check } from 'lucide-react';
...
export default function LoginPage() {
  ...
  const tabParam = (searchParams.get('tab') || 'commander').toLowerCase();
  const [activeTab, setActiveTab] = useState(tabParam); // 'commander' | 'victim' | 'admin'

  const [email, setEmail] = useState('operator@resq.gov');
  ...
  useEffect(() => {
    if (tabParam === 'admin') {
      ...
      setActiveTab('victim');
      setEmail('citizen@resq.gov');
      setPassword('citizen123');
    } else {
      setActiveTab('commander');
      setEmail('operator@resq.gov');
  ...
  useEffect(() => {
    if (isAuthenticated) {
      ...
      else if (role === 'COMMANDER' || role === 'OPERATOR') navigate('/commander/dashboard');
